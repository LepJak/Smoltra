...
const OrderListItem = (props) => {
    var date = "";
    if (props?.order?.created != null)
       date = format(new Date(props?.order?.created), 'dd.MM.yyyy hh:mm')
    const state = orderStateHelper.getState(props.order.state);
    console.log(props?.order?.dateCreated);
    return (<>
        <a style={{textDecoration:"none", color:"black"}} href={`/updateOrder/${props?.order?.id}`}>
